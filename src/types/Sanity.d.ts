...
declare global {
	namespace Sanity {
		...
		// Add the Music type here
		type Music = SanityDocument<{
			title: string
			artist: string
			spotifyLink: string
			coverImage?: Image
		...
		// Define the RecentMusic type
		type RecentMusicModule = SanityDocument<{
			recentMusic: Music[]; // Assuming recentMusic is an array of Music
		}> & {
			readonly _type: 'recent-music';
		};
		...
		type RecentMusicModule = Module<'recent-music'> & {
			recentMusic: Music[]
		}
	}
}
